...
myReverse([],[]).
myReverse([Elem|Tail],List):- myReverse(Tail, Rev), myConcat(Rev,[Elem],List). 
...
myReverseAcc([Elem|Tail], Acc, Result):- Acc1 = [Elem|Acc], myReverseAcc(Tail, Acc1, Result).
...
extract([],_,_,[]).
extract([Elem|Tail], 0, End, [Elem|Range]) :- End1 is End - 1,
                                        extract(Tail, 0, End1, Range).
extract([_|Tail], Start, End, Range) :- Start1 is Start - 1,
                                    extract(Tail, Start1, End, Range).
...
descendants(Nod, Nb) :- arc(Nod, A), \+ ((arc(Nod, B), A \= B)), descendants(A, Nb2),
                        Nb is Nb2 + 1.
descendants(Nod, Nb) :- arc(Nod, A), arc(Nod, B), A \= B, descendants(A, Nb1), descendants(B, Nb2),
                        Nb is Nb1 + Nb2 + 2.
...
height(Nod, Nb) :- arc(Nod, N1), \+ ((arc(Nod, N2), N1 \= N2)), height(N1, Nb2), Nb is Nb2 + 1.

height(Nod, Nb) :- arc(Nod, N1), arc(Nod, N2), N1 \= N2, height(N1, Nb1), height(N2, Nb2),
                Nb is max(Nb1, Nb2) + 1.
...
isBalanced(Nod):- isLeaf(Nod).

isBalanced(Nod):- arc(Nod, A), \+ ((arc(Nod, B), A \= B)),
                    height(A, Nb1), Nb1 =:= 0.

isBalanced(Nod):- arc(Nod, N1), arc(Nod, N2), N1 \= N2,
                    height(N1, Nb1), height(N2, Nb2), 
                    (Nb1 =:= Nb2 ; Nb1 - 1 =:= Nb2; Nb1 =:= Nb2 - 1).
check15:-
    tests(15, [
